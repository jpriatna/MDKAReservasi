; (function ($) {
    $.fn.printPreview = function (options) {
        ...
        return elem.bind("click.printPreview", function () {
            ...
            var str = "<!DOCTYPE html><style>@media print { #printBtn { display: none; } }</style><html><head>" + headString + opt.style + "</head><body style:\"margin: 10px; \">";
            str += $(opt.obj2print)[0].outerHTML.replace(btnCode, '') +
                "<button style=\"border-radius: 5px ;margin-left: 10px ; margin-bottom: 20px; padding: 10px 15px; width: 10%; border: none; background-color: #4caf50; color: #FFFFFF; height: 37px;\" id=\"printBtn\" onclick=\"window.print();\">PRINT</button></body></html>";
